...
$check_inv = select($pdo,"SELECT * FROM `invoice_premium` WHERE `invoice`='$inv' AND `user_id`='$user_id' and `status`='Unpaid'  "); 
$check_inv->execute();
...
if(isset($_POST['masukkan'])){
	...
	if(!empty($image)){
		...
		eksekusi($pdo,"UPDATE `invoice_premium` SET `bukti`='$image',`status_bukti`='Menunggu Konfirmasi' WHERE `invoice`='$inv'   ");
...
<div class="card-block"><div class="table-responsive">
	...
	<br />
	<a  class="btn btn-danger" href="?show=member_premium" >Kembali Ke List</a> 
