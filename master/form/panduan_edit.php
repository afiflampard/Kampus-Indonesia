...
$check_data = select($pdo,"SELECT * FROM `faq_panduan` WHERE `id`='$id'  "); 
$check_data->execute();
...
$check_k = select($pdo,"SELECT * FROM `faq_panduan` WHERE `judul`='$judul'  and `id`<>'$id' "); 
$check_k->execute();
$row_k = row($check_k);
if($row_k >= 1){
	$alert = "danger";
	...
	eksekusi($pdo,"UPDATE `faq`  SET  `judul`='$judul',`text`='$isi' WHERE `id`='$id'  ");
	?> 
...
<div class="card">
<div class="card-header"><h5>Edit Panduan</h5></div>
...
<form method="post" enctype="multipart/form-data"> 
	...
	<br />
	<button type="submit" name="new" class="btn btn-success" >Edit Data Panduan</button>  
